...
namespace reflecs
{
	...
	public:
		...
		*/
		void remove(entity_id e_id)
		{
			...
			assert(instance_to_remove < m_component_pool.size); /// Should never happen
#endif // _DEBUG
	...
	private:
		...
		*/
		template<uint32_t Idx>
		struct destroy_trivial_wrapper
		{
			void operator()(component_manager<C>* manager, component_instance instance)
			{
				manager->destroy_trivial<Idx>(instance);
			}
		};
		...
		*/
		...
		struct generate_buffers_wrapper
		{
			...
			{
				manager->generate_buffers<Idx>(buffer, num_elements);
			}
		};
		...
		void generate_buffers(void* buffer[], size_t num_elements)
		{
			...
			char* current_address = (char*)buffer[Idx + 1];

			size_t byte_count = current_address - previous_address;
		...
		*/
		...
		{
			void operator()(component_manager<C>* manager, uint32_t& bytes)
			{
				manager->count_component_size<Idx>(bytes);
			}
		};
		...
		*/
		...
		struct add_component_data_wrappper
		{
			...
			{
				mgr->add_component_data<Idx>(instance_to_add, component);
			}
		};
		...
		*/
		...
		struct remove_component_data_wrapper
		{
			...
			{
				mgr->remove_component_data<Idx>(instance_to_remove, replacing_instance);
			}
		};
	...
	private:
		 ...
		 */
		template<typename C, uint32_t elements>
		struct component_pool
		{
			uint32_t size = 1; // first available element in the array starts at 1; 0 reserved for error handling
			void* mem_buffer[elements] {};
			~component_pool()
			{
				free(mem_buffer[0]);
			}
